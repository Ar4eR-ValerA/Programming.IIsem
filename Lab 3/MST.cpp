#ifndef LAB_3_MST_CPP
#define LAB_3_MST_CPP
...
double MST(const vector <pair <double, double>> &coodinates) {
    ...
    for (int i = 0; i < coodinates.size(); ++i) {
        vertexes.push_back({coodinates[i].first, coodinates[i].second, i, numeric_limits <double>::max(), false});
    }
